...
created with a different persistent workspace, port publishing, or resource
limits, Affent refuses to silently reuse it; run `make image-serve-restart` to
recreate the container with the requested workspace and limits and wait for
`/healthz`.
