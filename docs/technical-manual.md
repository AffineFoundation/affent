...
go run ./cmd/affenteval --suite long-run --runtime-tools workspace,recall,plan --min-pass-rate 0.8 --min-completion-rate 0.9 --min-session-search-context-hit-rate 0.5 --max-tool-error-rate 0.05 --max-loop-guard-intervention-rate 0.1 --max-tool-context-truncation-rate 0.2 --max-avg-runtime-errors 0 --max-avg-context-compactions 1 --max-avg-total-tokens 120000
```
