...
This manual holds the operational details that used to live in the README:
build commands, runtime modes, Docker paths, server usage, configuration,
state, memory, tools, and integrations. Start with the root README for the
...
Its supported integration surfaces are the CLI, HTTP server, configuration,
state layout, and event streams. The internal Go loop is not treated as a
stable embeddable SDK surface.
...
Affent is built around those concerns. It keeps the agent runtime cycle
explicit, the tool surface configurable, and the persistent state inspectable on
disk.
