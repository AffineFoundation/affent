...
func TestResearchProfileGuidesGeneralExternalResearch(t *testing.T) {
	profile := researchProfile()
	for _, want := range []string{"registered external lookup tools", "authoritative sources", "Source hint", "llms.txt", "Direct-reader warning", "Embedded data preview", "page-source evidence", "Preserve user-provided disambiguators", "network/subnet id", "market, metrics, or trend questions", "social posts", "independent corroborating source", "API/text/export endpoints"} {
