...
- Run test/build commands directly. If the user or task names an exact verification command, use that command instead of substituting a nearby framework or runner. Do not wrap the first reproduction in cd ... && ... | head, ; echo "$?", or || true.
