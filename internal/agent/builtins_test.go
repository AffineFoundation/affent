...
func TestBuiltinFileToolErrorsUseWorkspaceRelativePaths(t *testing.T) {
	ctx := context.Background()
	ws := filepath.Join(t.TempDir(), "workspace")
	target := filepath.Join(ws, "app", "main.py")
	deps := BuiltinDeps{HostWorkspaceDir: ws}
	...
	read := readFileTool(deps)
	_, err := read.Execute(ctx, json.RawMessage(fmt.Sprintf(`{"path":%q}`, target)))
	if err == nil {
...
func TestBuiltinFileOpsErrorsUseWorkspaceRelativePaths(t *testing.T) {
	...
	ws := filepath.Join(t.TempDir(), "workspace")
	target := filepath.Join(ws, "app", "main.py")
	fake.files[target] = "package main\n"
	deps := BuiltinDeps{
		Executor:         fake,
		HostWorkspaceDir: ws,
	}
	...
	read := readFileTool(deps)
	_, err := read.Execute(ctx, json.RawMessage(fmt.Sprintf(`{"path":%q}`, filepath.Join(ws, "docs", "missing.md"))))
	if err == nil {
		t.Fatal("expected read_file missing file error")
	}
...
func TestSkillToolListsAndReadsEmbeddedSkills(t *testing.T) {
	...
	for _, want := range []string{"web_snapshot_fact_extraction", "coding_repair_workflow", "embed:internal/agent/builtin_skills", `"required_tools"`} {
		if !strings.Contains(list, want) {
			t.Fatalf("skill list missing %q:\n%s", want, list)
		}
	}
	if !strings.Contains(list, `"auto_activation"`) ||
		!strings.Contains(list, `"auto_activates": true`) ||
		!strings.Contains(list, `"activation": "`) {
...
func TestSkillToolInstallsRuntimeSkillWithoutRestart(t *testing.T) {
	...
	if !strings.Contains(list, `"runtime demo"`) || !strings.Contains(list, `"auto_activation"`) ||
		!strings.Contains(list, `"auto_activates": true`) ||
		!strings.Contains(list, `"activation": "any(runtime demo)"`) ||
		!strings.Contains(list, `"required_tools"`) {
	...
	if got := reg.ProvideForTools("please use runtime demo now", NewRegistry()); got != "" {
		t.Fatalf("installed skill should not activate when required tools are absent, got %q", got)
	}
	tools := NewRegistry()
	tools.Add(&Tool{Name: "memory"})
	if got := reg.ProvideForTools("please use runtime demo now", tools); !strings.Contains(got, body) {
		t.Fatalf("installed skill should activate when required tools are present, got %q", got)
	}
...
func TestSkillToolInstallReportsInactiveWithoutActivationRules(t *testing.T) {
	...
	if !strings.Contains(list, `"manual_demo"`) ||
		!strings.Contains(list, `"auto_activates": false`) ||
		!strings.Contains(list, `"activation": "none"`) {
