...
func renderTimelineScenarioExpectations(b *strings.Builder, scenario BatchScenario, ok bool) {
	...
	if exp.SessionID != "" || exp.ExecutePlan || exp.EnableMemory || exp.RequiredTurnEndReason != "" || exp.MaxTurns > 0 || exp.RuntimeMaxTurnInputTokens > 0 || exp.MaxTotalTokens > 0 || exp.MaxLoopTurnInputTokens > 0 || exp.MaxLoopTurnTotalTokens > 0 || exp.CompactTrigger > 0 || exp.CompactTriggerInputTokens != 0 || exp.ModelContextWindowTokens > 0 || exp.CompactTriggerInputPercent > 0 || exp.CompactKeepLast > 0 {
		var parts []string
		if exp.SessionID != "" {
		...
		if exp.MaxTotalTokens > 0 {
			parts = append(parts, fmt.Sprintf("max_total_tokens=%d", exp.MaxTotalTokens))
		}
	...
	if exp.RequiredContextMaintenance > 0 || len(exp.RequiredContextMaintenanceReasons) > 0 || exp.RequiredContextCompactions > 0 || exp.RequiredReactiveCompactions > 0 || len(exp.RequiredContextCompactionReasons) > 0 || exp.RequiredCompactionRemovedMsgs > 0 || exp.RequiredCompactionReducedBytes > 0 || exp.RequiredCompactScopeActive > 0 || strings.TrimSpace(exp.RequiredRuntimeCompactPrefillSource) != "" || strings.TrimSpace(exp.RequiredRuntimeSurfaceRefreshReason) != "" || len(exp.RequiredRuntimeSurfaceRefreshReasons) > 0 || strings.TrimSpace(exp.RequiredRuntimeLoopProtocolControl) != "" || exp.MaxCompactScopedPressurePercent != nil || len(exp.RequiredContextSummaryText) > 0 || len(exp.RequiredContextLoopProtocolAnchorText) > 0 {
		...
		if reason := strings.TrimSpace(exp.RequiredRuntimeSurfaceRefreshReason); reason != "" {
			parts = append(parts, fmt.Sprintf("runtime_surface_refresh_reason=%s", reason))
		}
		...
		writeTimelineCountsLine(b, "required_context_maintenance_reasons", exp.RequiredContextMaintenanceReasons)
...
func hasTimelineScenarioExpectations(exp DebugScenarioExpectations) bool {
	return len(exp.Suites) > 0 ||
		...
		exp.RequiredContextMaintenance > 0 ||
		len(exp.RequiredContextMaintenanceReasons) > 0 ||
