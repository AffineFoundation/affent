...
func TestParseTraceFileReadsToolRequestsAndFinalText(t *testing.T) {
	...
	if len(feeds.Examples) != 1 || feeds.Examples[0].LoopID != "longrun" || feeds.Examples[0].Mode != "digest" || feeds.Examples[0].CalibrationAnswers != 1 || feeds.Examples[0].LastCalibrationAnswer != "Stop if browser network evidence is missing." || feeds.Examples[0].PlanCurrentStep != "verify browser network evidence" || feeds.Examples[0].LastTurnID != "turn_previous" || feeds.Examples[0].LastTurnMemorySearchCalls != 3 || feeds.Examples[0].LastTurnMemorySearchMisses != 2 || feeds.Examples[0].LastTurnSessionSearchCalls != 1 || !strings.Contains(feeds.Examples[0].CurrentSituation, "dashboard metrics are partial") {
