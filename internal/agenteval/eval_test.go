...
func TestSelectLiveWebSuite(t *testing.T) {
	...
	for _, want := range []string{"SourceAccess:", "browser_network_url=", "requested_url=", "ref=", "source_method=network_xhr_fetch"} {
	...
	for _, want := range []string{"browser_network_url", "requested_url", "ref=", "source_method"} {
	...
	for _, want := range []string{"web_fetch", "browser_network_url", "requested_url", "ref=", "source_method"} {
...
func TestWriteScenarioDebugArtifactsIndexesTraceAndFinalText(t *testing.T) {
	...
	for _, want := range []string{
		...
		"tool#2 `browser_network_read` status=`network` url=`https://taostats.io/api/subnets/120` requested=`https://taostats.io/subnets/120` ref=`n1` source_method=`network_xhr_fetch` json_path=`$.price`",
