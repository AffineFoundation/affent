...
func TaskStateScheduleKindIs(kind string) Check {
	...
	return Check{
		...
		Eval: func(t Trace) CheckResult {
			...
			return CheckResult{
				Pass:   false,
				Detail: fmt.Sprintf("expected task_state schedule_kind %q, got %q", kind, got),
			}
		},
	}
}
		...
		Eval: func(t Trace) CheckResult {
			...
			for _, surface := range t.RuntimeSurfaces {
				if surface.MaxTurnInputTokens > 0 {
					observed = append(observed, surface.MaxTurnInputTokens)
				}
				if surface.MaxTurnInputTokens == expected {
					return CheckResult{Pass: true, Detail: fmt.Sprintf("max_turn_input_tokens=%d", expected)}
				}
			}
			return CheckResult{
				Pass:   false,
				Detail: fmt.Sprintf("expected runtime.surface max_turn_input_tokens=%d; observed=%v", expected, observed),
			}
		},
	}
}
...
func RuntimeSurfaceModelContextWindowSource(expected string) Check {
	expected = strings.TrimSpace(expected)
	return Check{
		Name: "runtime_surface_model_context_window_source:" + previewSubstr(expected, 48),
		Eval: func(t Trace) CheckResult {
			if expected == "" {
				return CheckResult{Pass: true}
			}
			...
			for _, surface := range t.RuntimeSurfaces {
				if surface.ModelContextWindowSource != "" {
					observed = append(observed, surface.ModelContextWindowSource)
				}
				if surface.ModelContextWindowSource == expected {
					return CheckResult{Pass: true, Detail: "model_context_window_source=" + expected}
				}
			}
			return CheckResult{
				Pass:   false,
				Detail: fmt.Sprintf("expected runtime.surface model_context_window_source=%q; observed=%v", expected, observed),
			}
		},
	}
}
		...
		Eval: func(t Trace) CheckResult {
			...
			for _, surface := range t.RuntimeSurfaces {
				if surface.ReservedOutputTokens > 0 {
					observed = append(observed, surface.ReservedOutputTokens)
				}
				if surface.ReservedOutputTokens == expected {
					return CheckResult{Pass: true, Detail: fmt.Sprintf("reserved_output_tokens=%d", expected)}
				}
			}
			return CheckResult{
				Pass:   false,
				Detail: fmt.Sprintf("expected runtime.surface reserved_output_tokens=%d; observed=%v", expected, observed),
			}
		},
	}
}
