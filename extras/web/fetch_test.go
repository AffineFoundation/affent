...
func TestSearchTool_AnnotatesDirectFetchRiskyResults(t *testing.T) {
	...
	for _, want := range []string{
		...
		"Direct-reader warning: do not open this URL as source evidence",
		"usually blocks page readers",
...
func TestDirectFetchCautionClassifiesGenericHosts(t *testing.T) {
	cases := []struct {
		...
		{name: "coingecko", url: "https://coingecko.com/en/coins", want: "usually blocks page readers"},
		{name: "x status", url: "https://x.com/affine/status/1", want: "do not open this URL as source evidence"},
