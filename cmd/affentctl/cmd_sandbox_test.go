...
func TestMakeImageServeEnablesBuiltinsInsideRuntimeContainer(t *testing.T) {
	...
	for _, want := range []string{
		...
		`ports: none`,
		`health=$$(curl -fsS "$(SERVE_HEALTH_URL)"`,
		`health=%s`,
		`health: unavailable ($(SERVE_HEALTH_URL))`,
		`build_revision={{index .Config.Labels "org.opencontainers.image.revision"}}`,
		`build_date={{index .Config.Labels "org.opencontainers.image.created"}}`,
		...
		`actual_build_revision=$$(docker inspect "$(SERVE_CONTAINER_NAME)" --format '{{index .Config.Labels "org.opencontainers.image.revision"}}'`,
		`was created from build_revision=$$actual_build_revision, but current checkout revision is $$expected_build_revision`,
		`run make image-serve-restart to recreate it with the current runtime image`,
...
func TestMakeOneClickContainerTargetsUseSharedLimits(t *testing.T) {
	...
	for target, wants := range map[string][]string{
		...
		"eval-container": {
			...
			`$(EVAL_RUNTIME_EVAL_MODE_ARGS) $(EVAL_RUNTIME_TOOLS_ARGS) $(EVAL_RUNTIME_ALL_TOOLS_ARGS) $(EVAL_RUNTIME_MEMORY_ARGS) $(EVAL_RUNTIME_WEB_ARGS) $(EVAL_RUNTIME_BROWSER_ARGS) $(EVAL_RUNTIME_MCP_CONFIG_ARGS) $(EVAL_TRACE_DELTAS_ARGS) $(EVAL_ARGS)`,
		},
...
func TestRuntimeForwardEnvIncludesPortableCLIAndServeConfig(t *testing.T) {
	...
	for _, name := range []string{
		...
		"AFFENTCTL_COMPACT_TRIGGER_INPUT_TOKENS",
		"AFFENTCTL_COMPACT_KEEP_LAST",
		"AFFENTCTL_TEMPERATURE",
		"AFFENTCTL_TOP_P",
		"AFFENTCTL_MAX_TOKENS",
		"AFFENTCTL_SEED",
		"AFFENTSERVE_MAX_SESSIONS",
		"AFFENTSERVE_SESSION_IDLE_TTL",
		"AFFENTSERVE_MAX_TURN_STEPS",
		"AFFENTSERVE_PER_CALL_TIMEOUT",
		"AFFENTSERVE_MAX_TRANSIENT_RETRIES",
		"AFFENTSERVE_RETRY_BACKOFF",
		"AFFENTSERVE_COMPACT_TRIGGER",
		"AFFENTSERVE_COMPACT_TRIGGER_INPUT_TOKENS",
		"AFFENTSERVE_COMPACT_KEEP_LAST",
		"AFFENTSERVE_BROWSER",
