...
func createCurrentSessionLoopDraft(b *loopBundle, goal string) error {
	...
	if b.loop != nil {
		...
		if !b.loopProtocolDraftInstalled {
			b.loop.SkillProvider = agent.WithLoopProtocolDraftActivationProvider(path, b.loop.SkillProvider)
			b.loopProtocolDraftInstalled = true
		}
	}
	return nil
}
...
func installCurrentSessionLoopProtocol(b *loopBundle) {
	...
	if !b.loopProtocolSkillInstalled {
		if agent.LoopProtocolRequiresCloseBeforeFinal(b.loopProtocolPath) {
