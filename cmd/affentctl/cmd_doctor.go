...
func doctorCapabilitySummary(c commonFlags) string {
	...
	return fmt.Sprintf(
		"shell_file=%t skill_install=%t memory=%t memory_only=%t eval_mode=%t session_search=%t project_context=%t symbol_context=%t repo_search=%t web_fetch=%t web_search=%t browser=%t mcp=%t subagent=%t subagent_max_depth=%d focused_tasks=%t focused_task_profiles=%s executor=%s",
		caps.Builtins,
		caps.Skill,
		caps.Memory,
		c.memoryOnly,
...
func doctorBoundarySummary(c commonFlags) string {
	...
	return fmt.Sprintf(
		"prompt_input=%s system_prompt=%s config=%s max_turns=%d call_timeout=%s llm_request=%s llm_error_body=%s stream_content=%s stream_reasoning=%s stream_tool_args=%s stream_tool_calls=%d stream_scanner=%s tool_args_event=%s tool_arg_string=%s tool_result_context=%s tool_result_context_budget=%s tool_result_event=%s tool_result_preview=%s repairable_tool_args=%s project_context=%s loop_guard_identical_calls=%d loop_guard_failure_warn=%d loop_guard_failure_halt=%d loop_guard_browser_find_no_match=%d plan_per_turn_calls=%d plan_steps=%d plan_step_text=%s plan_note=%s plan_evidence_refs=%d plan_evidence_ref=%s plan_state=%s active_plan_step=%s active_plan_note=%s active_plan_evidence_refs=%d active_plan_evidence_ref=%s focused_task_default_turns=%d focused_task_max_turns=%d focused_task_per_turn_calls=%d focused_task_type=%s focused_task_objective=%s focused_task_tool_result=%s focused_task_summary=%s focused_task_finding_evidence=%s focused_task_findings=%d focused_task_list_entries=%d focused_task_tool_calls=%d subagent_default_turns=%d subagent_max_turns=%d subagent_task=%s subagent_mode=%s subagent_tool_result=%s subagent_default_depth=%d subagent_hard_max_depth=%d skill_action=%s skill_name=%s skill_description=%s skill_body=%s skill_source=%s skill_triggers=%d skill_trigger=%s skill_required_tools=%d skill_required_tool=%s runtime_skills=%d runtime_skill_dir_read_batch=%d runtime_skill_manifest=%s runtime_skill_proposal=%s runtime_skill_proposal_id=%s mcp_result=%s mcp_http_json=%s mcp_http_sse_line=%s mcp_stdio_frame=%s jsonl_record=%s memory_file=%s memory_search_query=%s memory_search_terms=%d memory_search_snippet=%d memory_response_entry=%d",
		...
		formatBytes(ab.ToolResultContextBudgetBytes),
		...
		ab.LoopGuardIdenticalCalls,
		ab.LoopGuardFailureWarn,
		ab.LoopGuardFailureHalt,
		ab.LoopGuardBrowserFindNoMatch,
		ab.PlanPerTurnCalls,
		ab.PlanSteps,
		...
		ab.FocusedTaskDefaultTurns,
		ab.FocusedTaskMaxTurns,
		ab.FocusedTaskPerTurnCalls,
		formatBytes(ab.FocusedTaskTypeBytes),
		formatBytes(ab.FocusedTaskObjectiveBytes),
		formatBytes(ab.FocusedTaskToolResultBytes),
		formatBytes(ab.FocusedTaskSummaryBytes),
		formatBytes(ab.FocusedTaskFindingEvidenceBytes),
		ab.FocusedTaskFindings,
		ab.FocusedTaskListEntries,
		ab.FocusedTaskToolCalls,
		ab.SubagentDefaultTurns,
		ab.SubagentMaxTurns,
		formatBytes(ab.SubagentTaskBytes),
		formatBytes(ab.SubagentModeBytes),
		formatBytes(ab.SubagentToolResultBytes),
		ab.SubagentDefaultDepth,
		ab.SubagentHardMaxDepth,
		formatBytes(ab.SkillActionBytes),
		formatBytes(ab.SkillNameBytes),
		formatBytes(ab.SkillDescriptionBytes),
		formatBytes(ab.SkillBodyBytes),
		formatBytes(ab.SkillSourceBytes),
		ab.SkillTriggers,
		formatBytes(ab.SkillTriggerBytes),
		ab.SkillRequiredTools,
		formatBytes(ab.SkillRequiredToolBytes),
		ab.RuntimeSkills,
		ab.RuntimeSkillDirReadBatch,
		formatBytes(ab.RuntimeSkillManifestBytes),
		formatBytes(ab.RuntimeSkillProposalBytes),
		formatBytes(ab.RuntimeSkillProposalIDBytes),
