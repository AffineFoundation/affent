...
func TestRunListQualityProfiles(t *testing.T) {
	...
	for _, want := range []string{
		...
		"max-debug-brief-tag-rate=durable_completion=0.000",
		...
		"require-runtime-surface-refresh-reason=compact_window_observed,loop_protocol_changed,post_compaction,schedules_changed,workspace_changed",
	} {
		if !strings.Contains(out, want) {
...
func TestRunRejectsInvalidConfigBeforeScenarios(t *testing.T) {
	cases := []struct {
		...
		{
			name: "negative max scenario tokens",
			args: []string{"--max-scenario-total-tokens=-2"},
			want: "--max-scenario-total-tokens must be disabled with -1 or set to a non-negative value",
		},
		{
...
func TestQualityGateFailures(t *testing.T) {
	...
	summary := batchSummary{
		...
		MaxScenarioTotalTokens:                  250,
		MaxScenarioTokenScenario:                "token-heavy",
	...
	failures := qualityGateFailures(summary, qualityGateConfig{
		...
		MaxScenarioTotalTokens:                         ptr(200),
	...
	policyFailures := qualityGateFailures(batchSummary{
		Total:                           1,
		ContextCompactions:              2,
		ContextCompactionPolicyObserved: 1,
	}, qualityGateConfig{
		MinContextCompactionPolicyObservedRate: ptr(1),
	})
	if len(policyFailures) != 1 || !strings.Contains(policyFailures[0], "context_compaction_policy_observed_rate 0.500 < min 1.000") {
		t.Fatalf("policy observed-rate gate failures = %#v", policyFailures)
	}
}
...
func TestApplyQualityGateProfile(t *testing.T) {
	...
	if gates.MaxScenarioTotalTokens == nil || *gates.MaxScenarioTotalTokens != 240000 {
		t.Fatalf("longrun max scenario tokens = %#v, want 240000", gates.MaxScenarioTotalTokens)
	}
	...
	if !reflect.DeepEqual(gates.RequiredRuntimeSurfaceRefreshReasons, []string{"compact_window_observed", "loop_protocol_changed", "post_compaction", "schedules_changed", "workspace_changed"}) {
	...
	if gates.MaxDebugBriefTagRates["truncation:missing_artifact"] != 0 ||
		...
		gates.MaxDebugBriefTagRates["tool_failure:unclassified"] != 0 ||
		...
		t.Fatalf("longrun debug brief tag gates = %#v, want loop, recall, tool-failure classification, repair, verifier, workspace path, and truncation artifact gates", gates.MaxDebugBriefTagRates)
	}
...
func TestBatchSummaryAggregatesRuntimeMetrics(t *testing.T) {
	...
	summary.add(agenteval.BatchResult{
		...
		RuntimeSurface: &sse.RuntimeSurfacePayload{
			...
			EstimatedRequestInputTokens: 300,
...
func TestPrintBatchResultJSONLIncludesDebugPathsForRetainedWorkspace(t *testing.T) {
	...
	if surface["tool_count"] != float64(4) ||
		...
		surface["conversation_bytes"] != float64(12000) ||
		surface["tool_schema_bytes"] != float64(8000) ||
...
func TestPrintBatchSummaryJSONL(t *testing.T) {
	...
	for key, want := range map[string]any{
		...
		"avg_context_reduced_bytes":                   float64(0),
	...
	taskStateScheduleKinds, ok := got["task_state_by_schedule_kind"].(map[string]any)
	if !ok || taskStateScheduleKinds["checkin"] != float64(1) {
		t.Fatalf("task_state_by_schedule_kind = %#v\njson=%s", got["task_state_by_schedule_kind"], out.String())
	}
	...
	if got["runtime_surface_max_tool_schema_tokens"] != float64(500) ||
		...
		got["runtime_surface_max_tool_schema_pressure_percent"] != float64(125) {
		t.Fatalf("runtime surface request pressure not preserved: schema=%#v request=%#v pressure=%#v\njson=%s",
			got["runtime_surface_max_tool_schema_tokens"],
			got["runtime_surface_max_request_input_tokens"],
...
func TestEvalJSONLMetadataFromConfig(t *testing.T) {
	...
	minContextCompactionPolicyObservedRate := 0.95
	...
	maxScenarioTotalTokens := 240000.0
	...
	meta = evalJSONLMetadataFromConfig(" custom ", " flag-model ", " flag-provider ", " sandbox ", " 0.4 ", " 0.9 ", " 512 ", " 42 ", true, " readonly_workspace,web ", true, true, true, true, true, " /tmp/mcp.json ", time.Second, " Web-Evidence ", qualityGateConfig{
		...
		MinContextCompactionPolicyObservedRate:         &minContextCompactionPolicyObservedRate,
		...
		MaxScenarioTotalTokens:                         &maxScenarioTotalTokens,
...
func TestToolFailureKindHintIncludesWebSearchRecovery(t *testing.T) {
	for _, c := range []struct {
		...
		{kind: "loop_protocol_activation_status", want: "patch_draft for compact setup changes"},
