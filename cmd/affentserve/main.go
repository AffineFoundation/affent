...
func parseFlagsAndConfig(argv []string) (Config, error) {
	...
	var (
		...
		modelContextWindowAuto    = fs.Bool("model-context-window-auto", true, "Best-effort discovery of model context window from upstream /models metadata when explicit window is unset. Env: AFFENTSERVE_MODEL_CONTEXT_WINDOW_AUTO.")
	...
	if setFlags["compact-trigger-input-tokens"] {
		cfg.CompactTriggerInputTokens = *compactTriggerInputTokens
	}
	...
	if setFlags["eval-tools"] {
		cfg.EvalTools = *evalTools
	}
