...
EVAL_RUNTIME_WEB_ARGS = $(if $(filter true yes 1,$(EVAL_RUNTIME_WEB)),--runtime-web,)
EVAL_RUNTIME_BROWSER ?= false
EVAL_RUNTIME_BROWSER_ARGS = $(if $(filter true yes 1,$(EVAL_RUNTIME_BROWSER)),--runtime-browser,)
...
EVAL_TRACE_DELTAS ?= false
EVAL_TRACE_DELTAS_ARGS = $(if $(filter true yes 1,$(EVAL_TRACE_DELTAS)),--trace-deltas,)
...
image-serve-up:
	...
	if docker inspect "$(SERVE_CONTAINER_NAME)" >/dev/null 2>&1; then \
		...
		actual_serve_account_root=$$(docker inspect "$(SERVE_CONTAINER_NAME)" --format '{{index .Config.Labels "affent.runtime.serve.account_root"}}' 2>/dev/null); \
		if test "$$actual_listen" != "$(SERVE_LISTEN)" || test "$$actual_serve_workspace_root" != "$(SERVE_WORKSPACE_ROOT)" || test "$$actual_serve_memory_root" != "$(SERVE_MEMORY_ROOT)" || test "$$actual_serve_account_root" != "$(SERVE_ACCOUNT_ROOT)" ; then \
			echo "container $(SERVE_CONTAINER_NAME) was created with listen=$$actual_listen workspace_root=$$actual_serve_workspace_root memory_root=$$actual_serve_memory_root account_root=$$actual_serve_account_root, but requested listen=$(SERVE_LISTEN) workspace_root=$(SERVE_WORKSPACE_ROOT) memory_root=$(SERVE_MEMORY_ROOT) account_root=$(SERVE_ACCOUNT_ROOT)" >&2; \
